...
public class ViralImportDataHandler
{
    public async Task<Result<string>> ImportAsync(PatientData patientData)
    {
        ...
        {
            // Logger.Error($"Не удалось импортировать данные на платформу: {medArchive.Summary()}");
            return Result.Fail(medArchive.Summary());
        }
        ...
        {
            // Logger.Error($"Не удалось импортировать данные на платформу: {importResult.Summary()}");
            return Result.Fail(importResult.Summary());
        }

        if (!importResult.Value.Success)
        {
            // Logger.Error($"Не удалось импортировать данные на платформу: {importResult.Value.Error}");
        ...
        // Logger.Success($"На платформе в ресурсе \"Архив Историй MedIACP\" успешно создана новая ИБ {newIb.Name}");

        return Result.Ok(newIb.Name);
    }
        
    private DataSuccessor CreateViralHistory(PatientData patientData)
    {
        return new DataSuccessor()
        {
            ...
            {
                // Паспортная часть
                IacpaasDataConverter.GetPassportPath(patientData),
                // Дата обращения
    ...
    private long GetUnixTimeNumber() => (DateTime.UtcNow.Ticks -  new DateTime(1970, 1, 1).Ticks) / TimeSpan.TicksPerSecond;
}